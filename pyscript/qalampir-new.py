...
def fetch_sitemap():
    try:
        response = requests.get("https://qalampir.uz/post/latest-ajax?id=latest&limit=1020&offset=1020")
...
def process_articles(urls):
    ...
    for index, item in enumerate(urls):
        article_content = fetch_article_content(base_url+item)
        if article_content:
            enriched.append({
                "url": base_url + item,
                **article_content,
